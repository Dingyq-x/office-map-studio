...
const kindName = { space: '空间', furniture: '家具', seat: '座位', decoration: '装饰' } as const;
const icon = { space: '◇', furniture: '▣', seat: '●', decoration: '✦' } as const;
...
function meta(object: MapObject) {
  ...
  if (object.kind === 'furniture') return `${object.furnitureType || 'furniture'} · ${object.note || '办公室组件'}`;
  return `${object.decorationType || 'decoration'} · ${object.note || '装饰控件'}`;
}
...
export default function SearchBar({ query, results, onQueryChange, onSelect }: SearchBarProps) {
  ...
  return (
    <div className="search-box">
      ...
      <input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="搜索员工、部门、座位、空间、家具、装饰" />
