...
import { demoMapConfig } from './data/demoMapConfig';
import type { AssetTemplate, MapMode, MapObject, OfficeMapConfig } from './types/map';
import { searchObjects } from './utils/search';
import { loadConfig, loadObjects, resetObjects, saveConfig, saveObjects } from './utils/storage';

type ConfirmState = { type: 'reset' } | { type: 'delete'; id: string } | null;

const cloneDemo = () => demoMap.map((object) => ({ ...object }));
const cloneDemoConfig = (): OfficeMapConfig => ({ ...demoMapConfig });
...
export default function App() {
  ...
  const [mapConfig, setMapConfig] = useState<OfficeMapConfig>(() => loadConfig() ?? cloneDemoConfig());
  ...
  const addObject = (asset: AssetTemplate) => {
    ...
    const object: MapObject = {
      ...
      decorationType: asset.defaults.decorationType,
    ...
    <div className="app">
      <TopToolbar
        ...
        onSave={() => {
          ...
          showToast('布局和地图设置已保存到 localStorage');
        }}
      ...
      <div className="workspace">
        ...
        <MapCanvas
          ...
          onObjectResize={(id, width, height) => updateObject(id, { width, height })}
          onMapConfigChange={(patch) => setMapConfig((config) => ({ ...config, ...patch }))}
        />
        <div className="inspector-stack">
          <MapSettingsPanel config={mapConfig} mode={mode} onChange={(patch) => setMapConfig((config) => ({ ...config, ...patch }))} />
