...
function Sofa({ color }: { color: string }) {
  return (
    <group>
      ...
      <mesh castShadow position={[-0.62, 0.42, 0]}>
...
function Plant({ color }: { color: string }) {
  return (
    <group>
      ...
      {[[0, 0.58, 0], [0.14, 0.68, 0.04], [-0.14, 0.64, 0.03], [0.04, 0.76, -0.13], [-0.04, 0.72, 0.15]].map(([x, y, z], index) => (
...
export default function OfficeFurniture({ object, selected, highlighted, mode, onSelect, onMove }: OfficeFurnitureProps) {
